...
const NOMBRES_MASCULINOS = [
  'JUAN', 'CARLOS', 'JOSE', 'ANTONIO', 'FRANCISCO', 'MANUEL', 'DAVID', 'DANIEL', 
  'MIGUEL', 'RAFAEL', 'PEDRO', 'ANGEL', 'ALEJANDRO', 'FERNANDO', 'RICARDO', 
  'ROBERTO', 'EDUARDO', 'LUIS', 'JAVIER', 'JORGE', 'ALBERTO', 'DIEGO', 'MARIO',
  'SERGIO', 'PABLO', 'RAUL', 'GUILLERMO', 'IVAN', 'OSCAR', 'VICTOR', 'ARTURO',
  'RODRIGO', 'LEONARDO', 'EMILIO', 'GABRIEL', 'MARTIN', 'ANDRES', 'ADRIAN',
  'ENRIQUE', 'RUBEN', 'RAMON', 'NICOLAS', 'CRISTIAN', 'MAXIMILIANO', 'SANTIAGO'
];

const NOMBRES_FEMENINOS = [
  'MARIA', 'ANA', 'CARMEN', 'JOSEFA', 'ISABEL', 'ANTONIO', 'DOLORES', 'PILAR',
  'TERESA', 'ANGELES', 'MERCEDES', 'ROSA', 'FRANCISCA', 'CRISTINA', 'ANTONIA',
  'ELENA', 'CONCEPCION', 'MANUELA', 'SOLEDAD', 'AMPARO', 'SUSANA', 'JULIA',
  'MARGARITA', 'ESPERANZA', 'PIEDAD', 'NURIA', 'SILVIA', 'MONICA', 'PATRICIA',
  'BEATRIZ', 'VIRGINIA', 'ROCIO', 'INMACULADA', 'RAQUEL', 'MONTSERRAT', 'VICTORIA',
  'ANDREA', 'SOFIA', 'LUCIA', 'VALENTINA', 'CAMILA', 'ISABELLA', 'NATALIA',
  'PAOLA', 'GABRIELA', 'ALEJANDRA', 'CAROLINA', 'FERNANDA', 'LAURA'
];
...
export class PersonaUtils {
  ...
  static generarIniciales(nombres: string, apellidos: string): string {
    const inicialesNombres = nombres
      ? nombres.split(/\s+/).map(n => n.charAt(0).toUpperCase()).join('.')
      : '';
    
    const inicialesApellidos = apellidos
      ? apellidos.split(/\s+/).map(a => a.charAt(0).toUpperCase()).join('.')
      : '';
